...
public class Framebuffer : IDisposable
{
    ...
    public Framebuffer(int samples, PixelInternalFormat internalFormat = PixelInternalFormat.Rgba,
        PixelFormat format = PixelFormat.Rgba, PixelType pixelType = PixelType.UnsignedByte,
        int unboundFbo = 0, bool forceSingleSampleTexture = false)
    {
        GL.GetInteger(GetPName.MaxSamples, out _maxSamples);

        _unboundFbo = unboundFbo;
        _forceSingleSampleTexture = forceSingleSampleTexture;
    ...
    public void Init(int width, int height)
    {
        ...
        if (_forceSingleSampleTexture)
        {
            ...
            GL.TexImage2D(TextureTarget.Texture2D, 0, _internalFormat, width, height, 0, _format, _pixelType,
                IntPtr.Zero);
        ...
        else
        {
            ...
            GL.TexImage2DMultisample(TextureTargetMultisample.Texture2DMultisample, _samples, _internalFormat, width,
                height, true);
            GL.FramebufferTexture2D(FramebufferTarget.Framebuffer, FramebufferAttachment.ColorAttachment0,
        ...
        GL.BindTexture(_forceSingleSampleTexture ? TextureTarget.Texture2D : TextureTarget.Texture2DMultisample, 0);
        Release();
    }
